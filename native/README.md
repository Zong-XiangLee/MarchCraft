...
- Set/subset event timeline with directly editable transition counts, ripple insertion/deletion, variant-local performer groups, single-transition and whole-show playback, snapping, formations, axis locking, and undo/redo
...
- The timeline has aligned **Markers**, **Drill**, **Music**, and **Audio** lanes on one elapsed-time/count ruler. Narrow set markers are formation keyframes; the filled region before each destination marker is its incoming transition. The opening hold appears before music zero.
...
- Shift-click set markers for a contiguous range and Ctrl-click for discontiguous selection. Drag the marker lane for a count/time range; measures retain their own range selection. Seeking within an enabled loop preserves its range, while seeking outside disables looping without adding an undo entry.
- Drag the lower boundary handle to resize the incoming transition with live count, absolute-count, measure/beat, time, and snap-target feedback. Snap targets include counts, sets, measures, sections, tempo/meter changes, authored markers, and audio anchors; hold Alt for a temporary snap bypass. The separate dotted grip reorders sets and never changes duration.
...
- **Analyze music** proposes a non-destructive set plan from meter/tempo events, phrase-sized measure boundaries, authored sections/markers, MIDI ensemble/percussion attacks, MusicXML note activity, preferred transition lengths, and existing-set alignment. Sparse/Balanced/Detailed density and musical priority are configurable. Every suggestion reports evidence and confidence; accepted ghost markers can be toggled, added, removed, dragged, or given exact counts. **Apply** is one undoable command and **Cancel** leaves the project untouched. Existing sets are never moved automatically.
...
Project schema 12 stores movement-local timeline markers and timeline selection alongside the schema-11 movement data. Earlier projects open with their prior set range as the timeline selection and no authored markers; older app versions reject schema 12 rather than silently discard the new data. Analysis previews are intentionally transient and are never saved. MIDI synthesis and audio delivery run on a dedicated thread with precomputed event sample positions, independently of GUI animation. Playback position uses processed audio time, and gain/loop toggles preserve the active audio stream.
