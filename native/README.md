...
It supports ripple count editing, boundary dragging, desktop-style selection, authored
musical landmarks, and deterministic MIDI/MusicXML set-planning previews. Configurable opening
sets still match coordinate-sheet rows; a hold is represented by consecutive sets with identical coordinates.
...
- Tabbed performer/field configuration, optional step grid, step/yard display units, shape resize/rotation handles, and a time-scaled drill timeline with independent timing and reorder handles
...
- Native MIDI/MusicXML measure, meter, tempo, note-activity, and track import; unified drill/music/audio timeline; explainable local set-plan analysis; bundled FluidSynth MIDI playback; anchored rehearsal audio
...
- Right-click roster entries for the same group controls, or right-click set/transition events to edit incoming timing, insert/delete counts, copy, insert, archive, and edit sets. Unavailable commands remain visible in a muted disabled state.
...
- Use **Shape** to distribute selected performers on the full formation library; enable **Create as group** when desired. The non-modal Formation builder can be moved while the field remains interactive and closed with its × button. Assignment modes now have explicit contracts: Preserve spatial order never flips left/right or front/back relationships, Rehearsal safe prioritizes achievable collision-free travel, Shortest minimizes ensemble distance, Even effort minimizes the longest move, Feature favors readable reachable motion, and Roster uses fixed slots.
...
- The compact-by-default timeline has aligned **Markers**, **Drill**, **Music**, and **Audio** lanes on one elapsed-time/count ruler. Narrow set markers are formation keyframes; the filled region before each destination marker is its incoming transition. The opening hold appears before music zero, and the splitter can still expand the timeline for detailed work.
- Click a set marker to jump to its exact formation. Click a transition region to inspect its timing without changing the editing formation. Playback keeps running during ordinary navigation; double-click opens the set editor. Editing set, playback location, and animated transition destination remain separate states.
- Click or drag the ruler/playhead to scrub; release resumes only if playback was running. **Play** / Ctrl+Space resumes at the playhead. **Set tools → Play from selection** starts at the selected range's first set.
- Wheel-scroll all lanes together; Ctrl+wheel zooms around the pointer. **Fit show**, **Fit selection**, reveal-current-set, and **Follow** provide navigation without changing drill data. Level-of-detail hides labels and handles when they cannot be read.
- Shift-click either a set marker or its broad transition region for a contiguous page range; Ctrl-click toggles discontiguous pages. Drag the marker lane for a count/time range; measures retain their own range selection. Seeking within an enabled loop preserves its range, while seeking outside disables looping without adding an undo entry.
- **Snap: On** quantizes ruler scrubbing, time-range endpoints, plan-marker and authored-marker dragging, and transition resizing to counts or nearby sets, measures, sections, tempo/meter changes, authored markers, and audio anchors. A gold guide names the active target; hold Alt for a temporary bypass. The separate dotted grip reorders sets and never changes duration.
- Select a transition for exact count entry in either the compact toolbar or expanded contextual strip; the expanded strip also offers ±8 editing. During a boundary drag, all downstream drill regions and set markers preview their tick-accurate ripple while the music/audio lanes remain fixed. Release commits one undoable count edit; Escape or cancellation restores the original timing. The production sheet provides precise counts, absolute counts, timestamps, musical location, tempo, title/subset, and travel statistics. Inserting/deleting counts preserves all formation coordinates, variants, subsets, and authored transition paths. At narrow window sizes, **More…** holds secondary timeline actions instead of allowing controls to overlap.
...
Project schema 12 stores all movements, explicit per-performer transition start/duration counts, movement-local timeline markers, and timeline selection (including the selected marker identity) in the existing `.marchcraft` database. Earlier projects open as Movement 1 with their prior set range as the timeline selection and no authored markers; legacy `pathType="delayed"` data retains its 25% step-off behavior, and paths without timing fields use the complete transition. Older app versions reject schema 12 rather than silently discard the additional movement, transition, or timeline data. Analysis previews are intentionally transient and are never saved. MIDI synthesis and audio delivery run on a dedicated thread with precomputed event sample positions, independently of GUI animation. Playback position uses processed audio time, and gain/loop toggles preserve the active audio stream.
