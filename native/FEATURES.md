...
- Every multi-selection exposes image-editor-style uniform resize handles and a visible rotation control. Complete persistent formations retain their guide; partial edits detach only the edited members.
...
| Animation | Set-to-set or continuous whole-show playback with visible marcher paths, integer count-based step-offs, delayed curves, and per-set count/BPM timing |
...
| Drill editing | Transactional curve handles; preview/apply gate, pivot, and ordered follow tools for exact persistent groups; count-based step-offs and group staggering; arbitrary multi-selection scale/rotation; predictable Ctrl/Shift selection; axis lock, temporary snap bypass, grouping, keyboard nudge, per-set facing, persistent shapes, adaptive spirals, and equal-distance distribution |
...
- Transition paths support direct and quadratic curves plus persisted gate, field-pivot, and ordered follow group motions. Gate/pivot destinations remain unchanged until Apply; follow reports routes that cannot accommodate the group spacing or requested count offsets.
- Project format 11 stores destination `stepOffCount` independently from path geometry and stores group-transition identity, pivot/leader, direction, angle, order, interval, and leader route. Version-10 `delayed` paths migrate to direct paths with a 25%-of-transition delay.
