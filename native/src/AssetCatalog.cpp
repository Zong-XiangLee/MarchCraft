...
void AssetCatalog::validateAndInsert(const QVariantMap &asset)
{
    ...
    if (kind == QStringLiteral("bodyRig")) {
        ...
        const auto sockets = QSet<QString>(socketList.cbegin(), socketList.cend());
        const auto missing = RequiredSockets - sockets;
        if (!missing.isEmpty()) {
            m_errors.push_back(QStringLiteral("%1 is missing required sockets: %2")
...
QString AssetCatalog::fallbackId(const QString &kind) const
{
    ...
    if (kind == QStringLiteral("instrument")) return QStringLiteral("instrument.generic");
