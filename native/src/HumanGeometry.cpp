...
GeometryCache loadGeometry(int detailLevel)
{
    ...
    QFile catalog(QStringLiteral(":/catalog/catalog.json"));
    if (!catalog.open(QIODevice::ReadOnly)) {
        result.error = QStringLiteral("Human performer catalog is missing");
        return result;
    }
    const auto definition = QJsonDocument::fromJson(catalog.readAll()).object()
        .value(QStringLiteral("humanPerformer")).toObject();
    const auto lods = definition.value(QStringLiteral("runtimeLods")).toArray();
    if (lods.size() != 3 || !lods.at(detailLevel).toString().startsWith(QStringLiteral("qrc:/"))) {
        result.error = QStringLiteral("Human performer catalog has no valid runtime LODs");
        return result;
    }
    QFile file(lods.at(detailLevel).toString().mid(3));
